...
def setup_clusters_and_policies(args, megatron_config, vllm_config, tokenizer):
    ...
    vllm_inference_policy = VllmGeneration(
        cluster=generation_cluster, config=vllm_config
    )

    return megatron_cluster, generation_cluster, policy, vllm_inference_policy


def initialize_collective_refit(policy_cluster, generation_cluster, policy, generation):
    """Initialize collective communication for non-colocated refit."""
    print("\n--- Initializing collective refit ---")
    ip, port = policy_cluster.get_master_address_and_port()
    train_world_size = policy_cluster.world_size()
    inference_world_size = generation_cluster.world_size()
    ...
    futures_inference = generation.init_collective(
        ip,
        port,
        world_size=world_size,
        train_world_size=train_world_size,
...
def run_model_refitting(
    ...
    Args:
        ...
        colocated_inference: Whether policy and generation workers share a cluster
...
def main():
    ...
    try:
        ...
        if args.non_colocated:
            initialize_collective_refit(
                megatron_cluster, generation_cluster, policy, vllm_inference_policy
            )
        ...
        run_model_refitting(
            policy,
            vllm_inference_policy,
            args.refit_buffer_size_gb,
            colocated_inference=not args.non_colocated,
