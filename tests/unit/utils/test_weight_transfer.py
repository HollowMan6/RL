...
def _recorded_headers(broadcasted_tensors: list[torch.Tensor]) -> list[dict]:
    ...
    while index < len(broadcasted_tensors):
        kind, transport, payload_numel, metadata_len = (
            weight_transfer_mod._decode_header_control(broadcasted_tensors[index])
        )
        index += 1
        header = {
            "kind": kind,
...
def test_empty_transfer_header_uses_single_control_broadcast():
    ...
    header, refs = weight_transfer_mod._broadcast_header(
        {
            "kind": G_DELTA_UPDATE_KIND,
            "transport": weight_transfer_mod.G_DENSE_TRANSPORT,
            "payload_entries": [],
            "payload_numel": 0,
            "sparse_metadata": [],
        },
...
def test_sparse_indices_encoder_coalesces_small_tensors(monkeypatch):
    ...
    payload_tensors, _, metadata = weight_transfer_mod._encode_sparse_indices(tensors)
    decoded = dict(
        next(
            weight_transfer_mod._decode_sparse(
                payload_tensors,
                metadata,
                torch.device("cpu"),
                byte_cap=1024,
...
def test_sparse_indices_encoder_aliases_contiguous_views(monkeypatch):
    ...
    infos = [
        weight_transfer_mod._SparseTensorInfo(name, tensor, tensor.view(-1))
        for name, tensor in tensors
    ]

    aliased = weight_transfer_mod._alias_sparse_index_group(infos)
...
def test_sparse_indices_group_oom_falls_back_to_single_tensors(monkeypatch):
    monkeypatch.setenv("NRL_REFIT_SPARSE_ENCODE_COALESCE_BYTES", "1024")
    tensors = [
        ("weight_0", torch.tensor([0.0, 1.0, 0.0])),
        ("weight_1", torch.tensor([2.0, 0.0, 3.0])),
    ]
    real_sparse_indices_for_group = weight_transfer_mod._sparse_indices_for_group
...
def test_peer_chunk_advance_matches_chunk_boundaries_without_collecting_tensors():
    ...
    pending_item, exhausted = weight_transfer_mod._advance_chunk(
        iterator,
        byte_cap=20,
    )
...
def test_consumer_flushes_async_sparse_decode_before_full_update():
    ...
    sparse_delta[0] = 1.0
    sparse_tensors, transport, sparse_metadata = (
        weight_transfer_mod._encode_sparse_indices([("linear.weight", sparse_delta)])
    )
    ...
    full_header = {
        "kind": weight_transfer_mod.G_FULL_UPDATE_KIND,
        "transport": weight_transfer_mod.G_DENSE_TRANSPORT,
    ...
    for header, payload in (
        ...
        weight_transfer_mod._broadcast_header(
            header,
            group=group,
            src=0,
            device=torch.device("cpu"),
    ...
    with (
        _cpu_transfer_env(),
        patch("nemo_rl.utils.weight_transfer._decode_sparse", decode_sparse),
    ):
...
def test_high_density_sparse_delta_falls_back_to_full_update():
    tracker = DeltaCompressionTracker(_tracker_config())
    receiver_state = {}
    counters = {"full": 0, "delta": 0}
    initial = [
...
def test_memory_limited_stage_bytes_uses_fraction_of_free_cuda_memory(monkeypatch):
    monkeypatch.setattr(
        weight_transfer_mod.torch.cuda,
        "is_available",
        lambda: True,
    )
    monkeypatch.setattr(
    ...
    stage_bytes = weight_transfer_mod._memory_limited_stage_bytes(
        torch.device("cuda", 0),
        requested_bytes=512,
    )
...
def test_transfer_done_sync_does_not_synchronize_whole_device():
    ...
    with (
        ...
        patch(
            "nemo_rl.utils.weight_transfer.torch.cuda.current_stream",
            return_value=stream,
        ) as current_stream,
        patch("nemo_rl.utils.weight_transfer.torch.cuda.synchronize") as synchronize,
    ):
        _synchronize_current_transfer_stream(torch.device("cuda", 0))
...
def test_queued_sparse_payloads_wait_for_readiness_events_before_packing():
    ...
    with (
        ...
        patch(
            "nemo_rl.utils.weight_transfer._record_payload_readiness_events",
            side_effect=record_ready,
        ),
        patch(
            "nemo_rl.utils.weight_transfer._wait_for_payload_events",
