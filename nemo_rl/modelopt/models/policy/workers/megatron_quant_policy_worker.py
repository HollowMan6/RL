...
class MegatronQuantPolicyWorker(MegatronPolicyWorkerImpl):
    ...
    def _quantize(self, model):
        ...
        quantize_model(
            ...
            max_sample_length=self.cfg.get("quant_sequence_length", 1024),
        )
        return model
    ...
    def stream_weights_via_ipc_zmq(
        self, buffer_size_bytes: int = 0, kv_scales=None
    ) -> None:
        """Stream weights to vLLM.

        Colocated generation can offload Megatron params between refits, so
        real-quant export reloads them before building packed NVFP4 tensors.
        """
        if not self._use_real_quant_refit():
            return super().stream_weights_via_ipc_zmq(
        ...
        stream_weights_via_ipc_zmq_impl(
            params_generator=self._iter_real_quant_refit_params(kv_scales=kv_scales),
            buffer_size_bytes=buffer_size_bytes,
